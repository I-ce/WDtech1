...
/**
 * <pre> * author: Blankj * blog : http://blankj.com * time : 2016/9/21 * desc : 日志相关工具类 * </pre>
 */
public class LogUtils {
    ...
    /**
     * 初始化函数 * <p>与{@link #getBuilder()}两者选其一</p> * * @param logSwitch 日志总开关 * @param log2FileSwitch 日志写入文件开关，设为true需添加权限 {@code <uses-permission android:name="android.permission.WRITE_EXTERNAL_STORAGE"/>} * @param logFilter 输入日志类型有{@code v, d, i, w, e}<br>v代表输出所有信息，w则只输出警告... * @param tag 标签
     */
    public static void init(boolean logSwitch, boolean log2FileSwitch, char logFilter, String tag) {
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
            dir = App.getContext().getExternalCacheDir().getPath() + File.separator;
        } else {
            dir = App.getContext().getCacheDir().getPath() + File.separator;
        }
    ...
    public static Builder getBuilder() {
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
            dir = App.getContext().getExternalCacheDir().getPath() + File.separator + "log" + File.separator;
        } else {
            dir = App.getContext().getCacheDir().getPath() + File.separator + "log" + File.separator;
        }
        return new Builder();
    }
    ...
    private static void log(String tag, String msg, Throwable tr, char type) {
        ...
        while (index < str.length()) {
            ...
            if (logSwitch) {
                ...
                /*if (log2FileSwitch) {
                    log2File(type, generateTag(tag), sub.trim() + '\n' + Log.getStackTraceString(tr));
                }*/
            }
        }
    }
